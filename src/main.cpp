...
// Copyright 9c0 2014 KiTTYCoin Developers
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KiTTYCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // KiTTYCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KiTTYCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KiTTYCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KiTTYCoinMiner(CWallet *pwallet)
{
    printf("KiTTYCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running KiTTYCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KiTTYCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KiTTYCoinMiner, pwallet));
}
